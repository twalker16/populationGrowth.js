...
function nbYear(p0, percent, aug, p){
    ...
    p = Number(p)
    pArr = [p0]
    for(let i = 0; pArr[i] < p; i++){
        pArr.push((Math.round(pArr[i] * percent)+aug))
        // console.log(`Population after year ${i+1} is ${pArr[i+1]}`)
    }
    return `It took ${pArr.length - 1} years to reach a population over ${p}`
}
